...
use Pxpm\BackpackFullCalendar\Calendar as CalendarHelper;
...
class CalendarCrudController extends CrudController
{
	...
	public function show($id=null) {
        if ($id === null) {
            ...
            $calEvents = [];
            $calInfo = DBCalendar::where('id','=', $id)->with('calendareventsall.EventType')->first();
            $this->data['crud'] = $this->crud;

            $listCalEvents = $calInfo->calendareventsall;
            foreach ($listCalEvents as $eventCal) {
                $calEvents[] = CalendarHelper::event($eventCal->getTitle(),$eventCal->isAllDay(),$eventCal->getStart(),$eventCal->getEnd(),$eventCal->id,$eventCal->getEventOptions());
            }
            $this->data['calEventsFront'] = CalendarHelper::addEvents($calEvents);
            return view('viewCalendar', $this->data);
        }
    }
}
